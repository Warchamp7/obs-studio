...
class ThumbnailManager : public QObject {
	...
	ThumbnailCache thumbnailCache{32};
...
public:
	...
	ThumbnailView *createView(QWidget *parent, OBSSource &source);

private:
	Q_DISABLE_COPY_MOVE(ThumbnailManager);

	bool updateItem(ThumbnailItem *item);
	void updateNextItem(int cycleDepth = 0);

	void updateIntervalFromTotal(size_t newCount);

	void createThumbnailItem(const std::string &uuid, OBSSource &source);
	QPointer<ThumbnailItem> getThumbnailItem(const std::string &uuid, OBSSource &source);

private slots:
	void updateTick();
	void addToPriorityQueue(const std::string &uuid);
	void addItemToCache(std::string &uuid, QPixmap &pixmap);
	void deleteItemById(std::string uuid);
};
