...
void PluginManagerWindow::setupInstalledPage(std::vector<std::string> const &failedModules)
{
	...
	for (auto &metadata : modules_) {
		...
		bool isLoaded = moduleData != NULL;
		if (!isLoaded) {
			status = Status::Error;
			moduleData = obs_get_disabled_module(module_name.data());
		}

		bool isMissingModule = moduleData == NULL;
		if (isMissingModule) {
	...
	for (const std::string_view &moduleName : failedModules) {
		obs_module_t *moduleData = obs_get_disabled_module(moduleName.data());
		if (moduleData) {
			continue;
		}
