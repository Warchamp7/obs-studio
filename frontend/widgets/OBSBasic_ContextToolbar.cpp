	...
	if (item) {
		...
		if (contextBarSize >= ContextBarSize_Reduced && (updateNeeded || force)) {
			...
			if (flags & OBS_SOURCE_CONTROLLABLE_MEDIA) {
				...
				WindowCaptureToolbar *c = new WindowCaptureToolbar(ui->contextControlsContainer, source);
				c->Init();
				ui->contextControlsContainer->layout()->addWidget(c);

			} else if (strcmp(id, "monitor_capture") == 0 || strcmp(id, "display_capture") == 0 ||
				   strcmp(id, "xshm_input") == 0) {
				DisplayCaptureToolbar *c = new DisplayCaptureToolbar(ui->contextControlsContainer, source);
				c->Init();
				ui->contextControlsContainer->layout()->addWidget(c);

			} else if (strcmp(id, "dshow_input") == 0) {
				DeviceCaptureToolbar *c = new DeviceCaptureToolbar(ui->contextControlsContainer, source);
				ui->contextControlsContainer->layout()->addWidget(c);
